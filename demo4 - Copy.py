# ANALYTIX_AI FINAL CODE IMPLEMENTATION #
...
def process_doc_files(doc) -> dict:
...
# api_key = os.environ('put your API KEY')
...
all_docs = process_doc_files(doc)
...
if st.button('Execute Request'):
    if st.session_state.user_requests:
        ...
        for index, user_query in enumerate(st.session_state.user_requests):
            ...
            # Retrieve documents without using score_threshold ,'k': 1
            #"score_threshold": 0.65,
#             retriever = vectorstore.as_retriever(search_type="similarity", search_kwargs={'k': 1})
#             retrieved_doc_list = retriever.get_relevant_documents(user_query)

# # Manually filter the documents based on the score_threshold
#             score_threshold = 0.5
#             filtered_doc_list = [doc for doc in retrieved_doc_list if doc['score'] >= score_threshold]
            ...
            #st.write(generated_ai_output)
            exec_globals = {'df':df, 'plt':plt}
        
            exec(generated_ai_output, exec_globals)

            insight_question = "Extract interesting insights abouto this data in 30 words or less"

            for key, value in exec_globals.items():
                if isinstance(value, (pd.DataFrame, pd.Series)) and key != 'df':
                    wrangled_df = value

                    prompt2 = f"""{insight_question}
                    ...
                    generated_ai_output2 = generate_text(prompt = prompt2)

                    insights.append(generated_ai_output2)
    ...
    for index, insight in enumerate(insights):
        st.write(f"Chart {index +1}: {insight}", unsafe_allow_html = True)
        st.write('\n')
